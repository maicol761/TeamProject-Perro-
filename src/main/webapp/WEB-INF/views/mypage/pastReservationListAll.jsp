              ...
              <div class="col-sm-12">
                <div class="pagination font-alt">
		            <c:if test="${pageMaker.prev}">
		                <a href="PastreservationListAll${pageMaker.makeQuery(pageMaker.startPage-1)}"><i class="fa fa-angle-left"></i></a>
		            </c:if>   
		            <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="index">   
		            
		               <c:choose>
		         	   <c:when test="${criteria.pageNum==index }">
		         		   <a class="active" href="PastreservationListAll${pageMaker.makeQuery(index)}">${index}</a>
		          	   </c:when>
		               <c:otherwise>
		                	<a href="PastreservationListAll${pageMaker.makeQuery(index)}">${index}</a>
		               </c:otherwise>
		               </c:choose>
		            </c:forEach>
		          <c:if test="${pageMaker.next}">
		                <a href="PastreservationListAll${pageMaker.makeQuery(pageMaker.endPage+1)}"><i class="fa fa-angle-right"></i></a>
		          </c:if>	 
