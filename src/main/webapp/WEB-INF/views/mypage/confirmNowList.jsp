						...
						<div class="post">
							<div class="row mb-5">
								<table width="900" class="table table-hover">
									...
									<c:forEach items="${confirmNowList}" var="cNow">

										<tr>

											<td>${cNow.cono}</td>
											<td>${cNow.codate }</td>
											<td><fmt:formatNumber value="${cNow.copay }" type="currency"/>원</td>
											<td>${cNow.result }</td>
											<td><a href="confirm_view?cono=${cNow.cono}"
				...
				<div class="col-sm-12">
					<div class="pagination font-alt">
						<c:if test="${pageMaker.prev}">
							<a
								href="confirm_now_list${pageMaker.makeQuery(pageMaker.startPage-1)}"><i
						...
						<c:forEach begin="${pageMaker.startPage}"
							...
							<c:choose>
								<c:when test="${criteria.pageNum==index }">
									<a class="active"
										href="confirm_now_list${pageMaker.makeQuery(index)}">${index}</a>
								</c:when>
								<c:otherwise>
									<a href="confirm_now_list${pageMaker.makeQuery(index)}">${index}</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:if test="${pageMaker.next}">
							<a
								href="confirm_now_list${pageMaker.makeQuery(pageMaker.endPage+1)}"><i
